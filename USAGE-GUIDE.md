...
**组合搜索：**
```
请搜索项目654中路径包含"/user"且名称包含"登录"的接口
```
...
```
请获取项目654中接口ID为456的详细信息，包含完整的请求参数和返回格式
```
...
请创建到项目654的"视频播放页"分类中。
```

### 7. 批量创建接口

```
我需要为用户管理模块创建以下接口，都放在项目654的"个人中心页"分类中：
...
**批量更新接口状态：**
```
请将项目654中所有包含"/api/user"路径的接口状态改为"已完成"
```

**接口分类整理：**
```
请获取项目654的所有接口分类，并显示每个分类下有多少个接口
```
...
### 场景3：API Review 时
```
请获取项目654中所有状态为"开发中"的接口列表，我需要检查开发进度。
```
